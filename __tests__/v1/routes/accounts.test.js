...
describe('Accounts Routes', () => {
  ...
  beforeEach(() => {
    ...
    mockBudget = {
      ...
      createAccountGroup: jest.fn().mockResolvedValue('new-group-id'),
  ...
  describe('GET /budgets/:budgetSyncId/accountgroups', () => {
    ...
    it('should return list of account groups', async () => {
      const accountsModule = require('../../../src/v1/routes/accounts');
      accountsModule(mockRouter);
      ...
      expect(mockRes.json).toHaveBeenCalledWith({
        data: expect.arrayContaining([
          expect.objectContaining({ id: 'group1', name: 'Savings' })
        ])
      });
    });
  });
  ...
  describe('GET /budgets/:budgetSyncId/accountgroups/:accountGroupId', () => {
    it('should return specific account group', async () => {
      const accountsModule = require('../../../src/v1/routes/accounts');
      accountsModule(mockRouter);

      const handler = handlers['GET /budgets/:budgetSyncId/accountgroups/:accountGroupId'];
      ...
      expect(mockRes.json).toHaveBeenCalledWith({
        data: expect.objectContaining({ id: 'group1', name: 'Savings' })
      });
    });

    it('should reject for nonexistent account group', async () => {
      const accountsModule = require('../../../src/v1/routes/accounts');
      accountsModule(mockRouter);

      const handler = handlers['GET /budgets/:budgetSyncId/accountgroups/:accountGroupId'];

      mockReq.params.accountGroupId = 'missing';
      mockBudget.getAccountGroups.mockResolvedValueOnce([]);
  ...
  describe('POST /budgets/:budgetSyncId/accountgroups', () => {
    it('should create an account group', async () => {
      ...
      mockReq.body = {
        account_group: { name: 'Savings' }
      };

      await handler(mockReq, mockRes, mockNext);

      expect(mockBudget.createAccountGroup).toHaveBeenCalledWith(mockReq.body.account_group);
    ...
    it('should fail if account_group object is missing', async () => {
      const accountsModule = require('../../../src/v1/routes/accounts');
      accountsModule(mockRouter);
    ...
    it('should fail if account_group name is missing', async () => {
      const accountsModule = require('../../../src/v1/routes/accounts');
      accountsModule(mockRouter);
  ...
  describe('PATCH /budgets/:budgetSyncId/accountgroups/:accountGroupId', () => {
    it('should update an account group', async () => {
      ...
      mockReq.params.accountGroupId = 'group1';
      mockReq.body = {
        account_group: { name: 'New Savings Name' }
      };

      await handler(mockReq, mockRes, mockNext);

      expect(mockBudget.updateAccountGroup).toHaveBeenCalledWith('group1', mockReq.body.account_group);
